...
public static class BackgroundCompare
{
	...
	private static void MatchLines(List<Line> leftRange, List<Line> rightRange)
	{
		...
		Debug.Write($"{"".PadLeft(depth * 2)}- MatchLines {leftRange[0].LineIndex} -> {leftRange[^1].LineIndex}");
		...
		Debug.WriteLine($" - longest match {leftRange[matchIndex].LineIndex} -> {leftRange[matchIndex].LineIndex + matchLength - 1}");
	...
	private static void MatchPartialLines(List<Line> leftRange, List<Line> rightRange)
	{
		...
		Debug.Print($"{"".PadLeft(depth * 2)}- MatchPartialLines {leftRange[0].LineIndex} -> {leftRange[^1].LineIndex}");
